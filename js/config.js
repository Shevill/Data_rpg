const TRACKS = [
  ...
  { id:'ab',     name:'A/B Тесты',       emoji:'🧪', color:'#06b6d4', subtitle:'ЦПТ · ДИ · Гипотезы · Мощность',  subtitleEn:'CLT · CI · Hypotheses · Power',           requires:['ds'], requiresCount:3 },
];
...
const RECALL_ANSWERS={
  ...
  ds6:['rank(method="dense") не пропускает ранги при ничьих (1,1,2). method="min" — оба получают 1, следующий 3','resample("ME").mean() разбивает DatetimeIndex по месяцам и считает среднее. Нужен DatetimeIndex в df','shift(1) даёт значение предыдущего периода — lag-признак. Используй чтобы "прошлая продажа" предсказывала будущую','rolling(n).mean() ± 2*rolling(n).std() — коридор. Точки вне коридора — аномалии'],
  ds7:['MAE в единицах таргета, устойчив к выбросам. RMSE штрафует большие ошибки квадратом — выбери когда большие ошибки критичны','R²=0.72 → модель объясняет 72% дисперсии цены. 28% вариации осталось необъяснённым','Регуляризация уменьшает overfitting и мультиколлинеарность. Ridge→коэффициенты близки к нулю. Lasso→занулили незначимые признаки','Коэффициенты нестабильны и трудно интерпретировать: два коррелирующих признака "делят" вес непредсказуемо'],
  ab1:['ЦПТ: при n→∞ выборочные средние X̄ ~ N(μ, σ²/n) — нормальное, независимо от формы генеральной совокупности','SE делится на √n. При n×4 — SE÷2. Эффект квадратного корня — поэтому большие выборки дорого обходятся','SE доли = √(p(1-p)/n). Максимум при p=0.5 (SE≈0.5/√n). При p близком к 0 или 1 — SE меньше','ЦПТ позволяет строить ДИ и делать z-тест для любой метрики при достаточном n — не нужно знать распределение данных'],
  ab2:['При 95% ДИ: если повторить эксперимент 100 раз — ~95 интервалов содержат истинный параметр. Истинный параметр фиксирован','90%: 1.645, 95%: 1.960, 99%: 2.576. Мнемоника: 1.96 ≈ 2 — для быстрых расчётов в голове','При n×4 → SE÷2 → ширина ДИ ÷2. SE∝1/√n, ширина∝SE','0 внутри ДИ = нулевой эффект совместим с данными = нет значимого отличия от нуля'],
  ab3:['p-value = P(получить такой или более экстремальный результат | H₀ верна). Не вероятность того что H₀ верна!','I рода (α): ложная тревога, запустили плохую фичу. II рода (β): пропустили хорошую. В бизнесе часто α важнее','Каждый "взгляд" = тест. Если смотришь 10 раз → FWER растёт → ложные позитивы. Нужен Sequential Testing','При k метриках с α=0.05: P(хоть 1 ложная тревога) = 1-(0.95)^k. Бонферрони: α\'=α/k'],
  ab4:['Мощность 50% = как монетка. 80% — стандарт. 90% — если ошибка II рода дорого обходится','MDE — минимальный экономически значимый эффект. Бизнес говорит: "нам интересен рост от 2% конверсии". Статистик считает n','Досрочная остановка: если смотришь при каждых 100 событиях, реальный α вырастает. Нужен секвенциальный тест (SPRT)','Меньший α → нужен больший z → больший n. Больше мощность → нужен больший n. Оба требования — растёт n'],
  ab5:['AB boss — самооценка','AB boss — самооценка','AB boss — самооценка','AB boss — самооценка'],
};
...
const DATASETS={
  ...
  sql:[],
  ab:[
    {name:'Simulated A/B data (numpy)',url:'',desc:'import numpy as np; np.random.seed(42); n=5000; ctrl=np.random.binomial(n,0.10); treat=np.random.binomial(n,0.12)'},
    {name:'Cookie Cats A/B (Kaggle)',url:'https://www.kaggle.com/datasets/mursideyarkin/mobile-games-ab-testing-cookie-cats',desc:'Мобильная игра · gate_30 vs gate_40 · retention_1 и retention_7'},
  ]
};
